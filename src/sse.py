# Searchable Symmetric Encryption proposto por Song, Wagner e Perrig (2000)
...
L_BYTES = N_BYTES - M_BYTES  # (L =  64 bits)
...
def _aes_ecb_encrypt(key: bytes, block: bytes) -> bytes:
    """E(key, block): cifra determinística AES-128 ECB."""
...
def _aes_ecb_decrypt(key: bytes, block: bytes) -> bytes:
    """E_inverso(key, block): AES-128 ECB inverso."""
...
def _prg(k_seed: bytes, count: int) -> list[bytes]:
    ...
    nonce = b"\x00" * 16
    cipher = Cipher(algorithms.AES(k_seed), modes.CTR(nonce), backend=default_backend())
    enc = cipher.encryptor()
...
def keygen() -> tuple[bytes, bytes, bytes]:
    """KeyGen(): gera o trio de chaves secretas do esquema.
...
def search(C: list[bytes], trap: tuple[bytes, bytes]) -> list[int]:
    """Search(C, T_W): executado pelo servidor sobre o texto cifrado.
...
def decrypt(K: tuple, C: list[bytes]) -> list[str]:
    """Decrypt(K, C): recupera as palavras originais a partir do texto cifrado.
